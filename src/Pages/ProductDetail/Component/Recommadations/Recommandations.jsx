...
class Recommandations extends Component {
  ...
  goToPrevorNext = (e) => {
    ...
    fetch(
      `http://10.168.1.149:8000/product/best?limit=${LIMIT}&offset=${
        offset * LIMIT
      }`
    )
      .then((res) => res.json())
      ...
      <div className='Recc'>
        <div className='recomTop'>
          ...
          <div className='prevNextBtns'>
            <button className='prevBtns' onClick={this.goToPrevorNext}>
              &lt;
            </button>
            <button className='nextBtns' onClick={this.goToPrevorNext}>
              &gt;
            </button>
          </div>
        </div>
